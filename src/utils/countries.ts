...
export const countries = [
  ...
  new CountryData('Scotland', 'Scottish', 'GB-SCT', 'GB-SCT'),
  new CountryData('Wales', 'Welsh', 'GB-WLS', 'GB-WLS'),
  new CountryData('Northern Ireland', 'Northern Irish', 'GB-NIR', 'GB-NIR'),
  new CountryData('England', 'English', 'GB-ENG', 'GB-ENG'),
  new CountryData(
...
const formatInput = (input: string | null) =>
  input?.trim().replace(new RegExp(' ', 'g'), '').toLowerCase() ?? '';
export const findCountry = (input: string | null) => {
  const trimmedInput = formatInput(input);
  return countries.find(
    (c) => formatInput(c.name) === trimmedInput || formatInput(c.demonym) === trimmedInput
  );
};
